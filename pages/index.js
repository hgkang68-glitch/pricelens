// ?ìÑ pages/index.js ???∞ÏÉâ ?åÎßà + Í∞ùÎã®Í∞Ä ?úÏãú

import { useState, useEffect } from "react";
import Nav from "../components/Nav";

const PLATFORM_INFO = {
  naver:    { label: "?îç ?§Ïù¥Î≤??ºÌïë", color: "#059669" },
  coupang:  { label: "?õí Ïø†Ìå°",        color: "#DC2626" },
  eleventh: { label: "1Ô∏è‚É£ 11Î≤àÍ?",     color: "#EA580C" },
  gmarket:  { label: "?è™ GÎßàÏºì",       color: "#2563EB" },
};
...
export default function Home() {
  ...
  function handleItemClick(item, platformId) { setSelItem({ ...item, platformId }); setCompareData(searchData); }
  ...
  return (
    <div style={S.page}>
      ...
      {/* ?®Í≥Ñ ?úÏãú */}
      <div style={S.steps}>
        {["Ïπ¥ÌÖåÍ≥†Î¶¨","?ÅÌíà ?†ÌÉù","?®Îùº??Í≤Ä?âÍ≤∞Í≥?,"?åÎû´??Í∞ÄÍ≤?ÎπÑÍµê"].map((s, i) => (
      ...
      <div style={S.body}>
        ...
        {/* 1??*/}
        <div style={S.col}>
          <div style={S.colHead}>ÏΩîÏä§?∏ÏΩî ¬∑ {products.length}Í∞?/div>
          {loading && <div style={S.emptySmall}>Î∂àÎü¨?§Îäî Ï§?..</div>}
          {!loading && products.length === 0 && <div style={S.emptySmall}>?ÅÌíà ?ÜÏùå<br/>Í¥ÄÎ¶¨Ïûê ?òÏù¥ÏßÄ?êÏÑú ?±Î°ù?¥Ï£º?∏Ïöî</div>}
          {products.map(p => {
              ...
              <div key={p.id} style={{ ...S.prodCard, ...(costcoProd?.id === p.id ? S.prodCardOn : {}) }} onClick={() => handleCostcoClick(p)}>
                ...
                <div style={S.prodInfo}>
                  ...
                  <div style={S.prodMeta}>
                    <span style={S.prodBadge}>ÏΩîÏä§?∏ÏΩî</span>
        ...
        {/* 2??*/}
        <div style={S.col}>
          <div style={S.colHead}>?®Îùº??Í≤Ä?âÍ≤∞Í≥?costcoProd && <span style={S.colSub}> ¬∑ {costcoProd.name}</span>}</div>
          {!costcoProd && <div style={S.emptySmall}>???ÅÌíà???†ÌÉù?òÏÑ∏??/div>}
          {searchLoading && <div style={S.emptySmall}>?îÑ Í≤Ä??Ï§?..</div>}
                  ...
                  <div key={idx} style={{ ...S.searchItem, ...(selItem?.link === item.link ? S.searchItemOn : {}) }} onClick={() => handleItemClick(item, pid)}>
                    ...
                    <div style={S.searchInfo}>
                      ...
                      <div style={S.searchBottom}>
                        ...
                        {item.isFreeShip ? <span style={S.freeTag}>Î¨¥Î£åÎ∞∞ÏÜ°</span> : <span style={S.shipTag}>+??item.shippingFee?.toLocaleString()}</span>}
                        {item.isRocket && <span style={S.rocketTag}>?? Î°úÏºì</span>}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* 3??*/}
        <div style={S.col3}>
          {!selItem && <div style={S.empty}><div style={{ fontSize:32, marginBottom:8 }}>??/div><div style={{ fontSize:13, color:'#9CA3AF' }}>Í≤Ä?âÍ≤∞Í≥ºÏóê???ÅÌíà???†ÌÉù?òÏÑ∏??/div></div>}
            ...
            <div>
              ...
              <div style={S.selBox}>
                ...
                <div style={S.selInfo}>
                  ...
                  <div style={S.selMeta}>
                    ...
                    {costcoProd && <span style={S.selCostco}>ÏΩîÏä§?∏ÏΩî ??costcoProd.costco_price?.toLocaleString()}</span>}
                  </div>
                </div>
              </div>
              ...
              {compareData.winner && (
                <div style={S.winnerBox}>
                  <div style={S.winnerLeft}>
                    <span style={S.winnerBadge}>?èÜ ÏµúÏ?Í∞Ä</span>
                  ...
                  <div style={S.winnerRight}>
                    {compareData.winner.saving > 0 && <span style={S.chipGreen}>?Ä ?åÎû´?ºÎ≥¥????compareData.winner.saving?.toLocaleString()} ?Ä??/span>}
                    {costcoProd && compareData.winner.price < costcoProd.costco_price && <span style={S.chipGreen}>ÏΩîÏä§?∏ÏΩîÎ≥¥Îã§ ??(costcoProd.costco_price - compareData.winner.price)?.toLocaleString()} ?Ä???éâ</span>}
                    {costcoProd && compareData.winner.price >= costcoProd.costco_price && <span style={S.chipGray}>ÏΩîÏä§?∏ÏΩîÍ∞Ä ???Ä??/span>}
                  </div>
                </div>
              )}
              ...
              <div style={S.table}>
                <div style={S.tableHead}>
                  <div style={S.th}>?åÎû´??/div>
                  <div style={{ ...S.th, textAlign:'right' }}>?ÅÌíàÍ∞Ä</div>
                  <div style={{ ...S.th, textAlign:'right' }}>Î∞∞ÏÜ°Îπ?/div>
                  <div style={{ ...S.th, textAlign:'right' }}>Ï¥?Í≤∞Ï†ú??/div>
                </div>
                  ...
                  return (
                    <div key={pid} style={{ ...S.tableRow, ...(isBest ? S.tableRowBest : {}) }}>
                      <div style={{ ...S.td, display:'flex', alignItems:'center', gap:5 }}>
                        ...
                        {isBest && <span style={S.bestBadge}>ÏµúÏ?Í∞Ä</span>}
                        {(!platData?.items?.length) && <span style={S.errBadge}>ÎØ∏Ïó∞Í≤?/span>}
                      </div>
                      <div style={{ ...S.td, textAlign:'right', color: isBest ? '#059669' : '#111827', fontWeight: isBest ? 600 : 400 }}>{item ? `??{item.price?.toLocaleString()}` : '??}</div>
                      <div style={{ ...S.td, textAlign:'right', color: item?.isFreeShip ? '#059669' : '#6B7280' }}>{item ? (item.isFreeShip ? 'Î¨¥Î£å' + (item.isRocket?' ??':'') : `??{item.shippingFee?.toLocaleString()}`) : '??}</div>
                ...
                {costcoProd && (
                  <div style={{ ...S.tableRow, background:'#F9FAFB' }}>
                    <div style={S.td}><span style={{ fontSize:12, color:'#9CA3AF' }}>?è¨ ÏΩîÏä§?∏ÏΩî (Í∏∞Ï?)</span></div>
                    <div style={{ ...S.td, textAlign:'right', color:'#9CA3AF', fontSize:12 }}>??costcoProd.costco_price?.toLocaleString()}</div>
                    <div style={{ ...S.td, textAlign:'right', color:'#9CA3AF', fontSize:12 }}>ÏßÅÏ†ëÍµ¨Îß§</div>
                    <div style={{ ...S.td, textAlign:'right', color:'#9CA3AF', fontSize:12 }}>??costcoProd.costco_price?.toLocaleString()}</div>
                  </div>
                )}
              </div>

              <div style={S.note}>* Î∞∞ÏÜ°ÎπÑÎäî ?êÎß§??Ï°∞Í±¥???∞Îùº ?¨ÎùºÏß????àÏäµ?àÎã§</div>
              ...
              <div style={{ padding:'12px 16px', borderTop:'1px solid #F3F4F6', display:'flex', justifyContent:'flex-end' }}>
                <button onClick={handleSave} disabled={saveStatus==="saving"} style={{
                  ...
                  {saveStatus==="saving"?"?Ä??Ï§?..":saveStatus==="done"?"???Ä?•Îê®":saveStatus==="err"?"?Ä???§Ìå®":"?íæ ÎπÑÍµêÍ≤∞Í≥º ?Ä??}
                </button>
              </div>
            </div>
          )}
